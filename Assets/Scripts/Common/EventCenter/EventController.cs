...
public class EventController
{
    ...
    private void OnListenerAdding(string eventType, Delegate listenerBeingAdded)
    {
        ...
        {
            throw new Exception(string.Format("Try to add not correct event {0}. Current type is {1}, adding type is {2}.", eventType, delegate2.GetType().Name, listenerBeingAdded.GetType().Name));
        }
    }
    ...
    private bool OnListenerRemoving(string eventType, Delegate listenerBeingRemoved)
    {
        ...
        {
            throw new Exception(string.Format("Remove listener {0}\" failed, Current type is {1}, adding type is {2}.", eventType, delegate2.GetType(), listenerBeingRemoved.GetType()));
        }
        return true;
    }
