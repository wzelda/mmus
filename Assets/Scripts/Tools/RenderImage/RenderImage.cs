...
namespace LPCFramework {
    ...
    public class RenderImage {
        ...
        public static void SetRenderTexture (GGraph holder, Camera camera) {
            ...
            rt = RenderTexture.GetTemporary((int) holder.width, (int) holder.height, 24/* , RenderTextureFormat.RGB565 */);
            camera.targetTexture = rt;

            if (holder.displayObject.GetType () == typeof (Image)) {
                var img = holder.displayObject as Image;
                img.texture = new NTexture (rt);
                return;
            }
            holder.SetNativeObject (new Image { texture = new NTexture (rt) });
        }
    }
}