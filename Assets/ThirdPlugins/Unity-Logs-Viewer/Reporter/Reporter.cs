...
public class Reporter : MonoBehaviour
{
    ...
    public static void SetVisible()
    {
        ...
        else if (_instance.show)
        {
            _instance.show = false;
            ReporterGUI gui = _instance.GetComponent<ReporterGUI>();
            DestroyImmediate(gui);

            try
            {
                _instance.SendMessage("OnHideReporter");
            }
	...
	void drawToolBar()
	{
		toolBarRect.x = Screen.safeArea.x;
		toolBarRect.y = (Screen.height - Screen.safeArea.yMax);
		toolBarRect.width = Screen.safeArea.width;
		...
		GUI.skin = toolbarScrollerSkin;
		Vector2 drag = getDrag();
		if ((drag.x != 0) && (downPos != Vector2.zero) && (downPos.y > Screen.safeArea.yMax - size.y * 2f)) {
        ...
        if (showCopyAllButton)
        {
            if (GUILayout.Button(copyAllContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
            {
                ...
                if(allLogsToClipboard.Trim().Length == 0)
                    GUIUtility.systemCopyBuffer = "No log selected";
                else
	...
	public void OnGUIDraw()
	{
		...
		screenRect = Screen.safeArea;
		screenRect.y = Screen.height - Screen.safeArea.yMax;

		getDownPos();

		logsRect.x = Screen.safeArea.x;
		logsRect.y = size.y * 2f + (Screen.height - Screen.safeArea.yMax);
		logsRect.width = Screen.safeArea.width;
		logsRect.height = Screen.safeArea.height * 0.75f - size.y * 2f;

		stackRectTopLeft.x = 0f;
		stackRect.x = 0f;
		stackRectTopLeft.y = (Screen.height - Screen.safeArea.yMax) + Screen.safeArea.height * 0.75f;
		stackRect.y = (Screen.height - Screen.safeArea.yMax) + Screen.safeArea.height * 0.75f;
		stackRect.width = Screen.safeArea.width;
		stackRect.height = Screen.safeArea.y + Screen.safeArea.height * 0.25f - size.y;

		detailRect.x = 0f;
		detailRect.y = Screen.height - size.y * 3;
		detailRect.width = Screen.safeArea.width;
